...
fn sym_complex(id: &str, name: &str, kind: SymbolKind, file: &str, start: u32, end: u32, complexity: u32) -> Symbol {
    Symbol {
        id: id.to_string(),
        name: name.to_string(),
        kind,
...
fn setup_graph() -> TestGraph {
    ...
    let extractions = vec![
        FileExtraction {
            ...
            symbols: vec![
                sym("src/app.py::main", "main", SymbolKind::Function, "src/app.py", 1, 30),
                sym("src/app.py::helper", "helper", SymbolKind::Function, "src/app.py", 32, 50),
                sym_complex("src/app.py::complex_handler", "complex_handler", SymbolKind::Function, "src/app.py", 52, 200, 25),
            ],
            relations: vec![
                rel("src/app.py::main", "src/app.py::helper", RelationKind::Calls),
                rel("src/app.py::main", "src/app.py::complex_handler", RelationKind::Calls),
                rel("src/app.py::complex_handler", "src/app.py::helper", RelationKind::Calls),
            ],
            statements: vec![],
        },
        FileExtraction {
            ...
            symbols: vec![
                sym("src/models.py::BaseModel", "BaseModel", SymbolKind::Class, "src/models.py", 1, 20),
                sym("src/models.py::UserModel", "UserModel", SymbolKind::Class, "src/models.py", 22, 50),
                sym("src/models.py::UserModel::save", "save", SymbolKind::Method, "src/models.py", 30, 45),
            ],
            relations: vec![
                rel("src/models.py::UserModel", "src/models.py::BaseModel", RelationKind::Inherits),
                rel("src/app.py::complex_handler", "src/models.py::UserModel::save", RelationKind::Calls),
            ],
            statements: vec![],
        },
        FileExtraction {
            ...
            symbols: vec![
                sym("src/utils.py::format_date", "format_date", SymbolKind::Function, "src/utils.py", 1, 10),
                sym("src/utils.py::validate", "validate", SymbolKind::Function, "src/utils.py", 12, 25),
            ],
            relations: vec![],
            statements: vec![],
        },
...
fn test_cluster_isolated_symbols() {
    ...
    let conn = store.connection().unwrap();
    let stats = infigraph_core::cluster::detect_clusters(&conn).unwrap();
    assert!(stats.num_clusters >= 2, "isolated symbols should each be own cluster");
}
...
fn test_manifest_package_json() {
    ...
    assert!(results[0].deps.iter().any(|d| d.name == "express" && !d.is_dev));
...
"#,
    ...
    assert!(results[0].deps.iter().any(|d| d.name == "serde" && !d.is_dev));
    assert!(results[0].deps.iter().any(|d| d.name == "tempfile" && d.is_dev));
}
...
fn test_manifest_go_mod() {
    ...
    assert!(results[0].deps.iter().any(|d| d.name.contains("gin-gonic") && !d.is_dev));
    assert!(results[0].deps.iter().any(|d| d.name.contains("golang.org")),
...
fn test_manifest_pom_xml() {
  ...
  <dependencies>
    ...
    assert!(results[0].deps.iter().any(|d| d.name.contains("spring-core") && !d.is_dev));
    assert!(results[0].deps.iter().any(|d| d.name.contains("junit") && d.is_dev));
}
...
fn test_manifest_gemfile() {
    ...
    assert!(results[0].deps.iter().any(|d| d.name == "rails" && !d.is_dev));
    assert!(results[0].deps.iter().any(|d| d.name == "rspec" && d.is_dev));
}
...
fn test_manifest_composer_json() {
    ...
    assert!(results[0].deps.iter().any(|d| d.name.contains("laravel") && !d.is_dev));
    assert!(results[0].deps.iter().any(|d| d.name.contains("phpunit") && d.is_dev));
}
...
fn test_manifest_pubspec_yaml() {
    ...
    assert!(results[0].deps.iter().any(|d| d.name == "http" && !d.is_dev));
    assert!(results[0].deps.iter().any(|d| d.name == "flutter_test" && d.is_dev));
}
...
<packages>
  ...
  <package id="xunit" version="2.4.1" developmentDependency="true" />
    ...
    assert_eq!(results.len(), 1);
    assert!(results[0].deps.iter().any(|d| d.name == "Newtonsoft.Json" && !d.is_dev));
    assert!(results[0].deps.iter().any(|d| d.name == "xunit" && d.is_dev));
}
...
fn test_vuln_filter_by_severity() {
    let mut report = infigraph_core::vuln::VulnReport {
        ...
        findings: vec![
            infigraph_core::vuln::VulnEntry {
                dep_name: "a".to_string(), dep_version: "1.0".to_string(),
                ecosystem: "npm".to_string(), vuln_id: "V1".to_string(),
                summary: "".to_string(), severity: "CRITICAL".to_string(),
                fixed_version: None, url: "".to_string(),
            },
            infigraph_core::vuln::VulnEntry {
                dep_name: "b".to_string(), dep_version: "1.0".to_string(),
                ecosystem: "npm".to_string(), vuln_id: "V2".to_string(),
                summary: "".to_string(), severity: "HIGH".to_string(),
                fixed_version: None, url: "".to_string(),
            },
            infigraph_core::vuln::VulnEntry {
                dep_name: "c".to_string(), dep_version: "1.0".to_string(),
                ecosystem: "npm".to_string(), vuln_id: "V3".to_string(),
                summary: "".to_string(), severity: "LOW".to_string(),
                fixed_version: None, url: "".to_string(),
            },
        ],
    };
    ...
    assert!(report.findings.iter().all(|f| f.severity == "CRITICAL" || f.severity == "HIGH"));
}
...
fn test_vuln_filter_by_ecosystem() {
    let mut report = infigraph_core::vuln::VulnReport {
        ...
        findings: vec![
            ...
            infigraph_core::vuln::VulnEntry {
                dep_name: "b".to_string(), dep_version: "1.0".to_string(),
                ecosystem: "pip".to_string(), vuln_id: "V2".to_string(),
                summary: "".to_string(), severity: "HIGH".to_string(),
                fixed_version: None, url: "".to_string(),
            },
        ],
...
fn test_check_run_complexity() {
    ...
    let cfg = infigraph_core::check::CheckConfig {
        ...
        security: infigraph_core::check::SecurityConfig { enabled: false, ..Default::default() },
        dead_code: infigraph_core::check::DeadCodeConfig { enabled: false, ..Default::default() },
        vulnerabilities: infigraph_core::check::VulnCheckConfig { enabled: false, ..Default::default() },
    };
    let sel = infigraph_core::check::CheckSelection::from_csv("complexity");
    let results = infigraph_core::check::run_checks(
        tg._dir.path(), &cfg, &tg.store, &sel,
    );
    assert_eq!(results.len(), 1);
    assert_eq!(results[0].name, "complexity");
    assert_eq!(results[0].status, infigraph_core::check::CheckStatus::Fail,
...
fn test_check_run_dead_code() {
    ...
    let cfg = infigraph_core::check::CheckConfig {
        ...
        security: infigraph_core::check::SecurityConfig { enabled: false, ..Default::default() },
        complexity: infigraph_core::check::ComplexityConfig { enabled: false, ..Default::default() },
        vulnerabilities: infigraph_core::check::VulnCheckConfig { enabled: false, ..Default::default() },
    };
...
fn test_check_status_display() {
    assert_eq!(format!("{}", infigraph_core::check::CheckStatus::Pass), "PASS");
    assert_eq!(format!("{}", infigraph_core::check::CheckStatus::Fail), "FAIL");
}
...
fn test_refactor_analyze_empty_graph() {
    ...
    let recs = infigraph_core::refactor::analyze(
        &conn, None, None, infigraph_core::refactor::Focus::All, 10,
    ).unwrap();
...
fn test_refactor_analyze_detects_complexity() {
    ...
    let recs = infigraph_core::refactor::analyze(
        &conn, None, None, infigraph_core::refactor::Focus::Complexity, 10,
    ).unwrap();
    assert!(recs.iter().any(|r| r.target.contains("complex_handler")),
...
fn test_refactor_analyze_with_target_filter() {
    ...
    let recs = infigraph_core::refactor::analyze(
        &conn, None, Some("src/utils.py"), infigraph_core::refactor::Focus::All, 10,
    ).unwrap();
    for rec in &recs {
        assert!(rec.target.contains("utils.py") || !rec.target.contains("app.py"),
...
fn test_refactor_analyze_limit() {
    ...
    let recs = infigraph_core::refactor::analyze(
        &conn, None, None, infigraph_core::refactor::Focus::All, 1,
    ).unwrap();
    assert!(recs.len() <= 1);
}
...
fn test_refactor_format_recommendations_populated() {
    let recs = vec![
        infigraph_core::refactor::Recommendation {
...
fn test_refactor_focus_from_str() {
    assert_eq!(infigraph_core::refactor::Focus::parse("complexity"), infigraph_core::refactor::Focus::Complexity);
    assert_eq!(infigraph_core::refactor::Focus::parse("duplication"), infigraph_core::refactor::Focus::Duplication);
    assert_eq!(infigraph_core::refactor::Focus::parse("coupling"), infigraph_core::refactor::Focus::Coupling);
    assert_eq!(infigraph_core::refactor::Focus::parse("size"), infigraph_core::refactor::Focus::Size);
    assert_eq!(infigraph_core::refactor::Focus::parse("unknown"), infigraph_core::refactor::Focus::All);
}

#[test]
fn test_refactor_category_display() {
    assert_eq!(format!("{}", infigraph_core::refactor::Category::SplitFile), "split_file");
    assert_eq!(format!("{}", infigraph_core::refactor::Category::ExtractFunction), "extract_function");
    assert_eq!(format!("{}", infigraph_core::refactor::Category::MergeDuplicates), "merge_duplicates");
    assert_eq!(format!("{}", infigraph_core::refactor::Category::RemoveDeadCode), "remove_dead_code");
    assert_eq!(format!("{}", infigraph_core::refactor::Category::ReduceCoupling), "reduce_coupling");
    assert_eq!(format!("{}", infigraph_core::refactor::Category::SimplifyLogic), "simplify_logic");
}

#[test]
fn test_refactor_impact_effort_display() {
    assert_eq!(format!("{}", infigraph_core::refactor::Impact::High), "high");
    assert_eq!(format!("{}", infigraph_core::refactor::Impact::Medium), "medium");
    assert_eq!(format!("{}", infigraph_core::refactor::Impact::Low), "low");
    assert_eq!(format!("{}", infigraph_core::refactor::Effort::High), "high");
    assert_eq!(format!("{}", infigraph_core::refactor::Effort::Medium), "medium");
