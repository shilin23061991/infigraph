...
fn main() -> Result<()> {
    ...
    eprintln!("Extracted {} files ({} skipped) in {}ms: {} symbols, {} relations, {} statements",
        extractions.len(), skipped, extract_ms, total_symbols, total_relations, total_stmts);
    ...
    eprintln!("open={}ms  write={}ms  stats={}ms", kuzu_open_ms, kuzu_write_ms, kuzu_stats_ms);
    eprintln!("{} symbols, {} calls, {} modules, {} files",
    ...
    eprintln!("open={}ms  write={}ms  counts={}ms", cozo_open_ms, cozo_write_ms, cozo_counts_ms);
    eprintln!("{} symbols, {} calls, {} modules, {} files",
        cozo_stats.symbols, cozo_counts.get("calls").copied().unwrap_or(0),
        cozo_stats.modules, cozo_stats.files);
    ...
    for (label, kuzu_n, cozo_n) in [
        ...
        if !ok { failures += 1; }
        eprintln!("{:<15} Kuzu={:<6} CozoDB={:<6} {}", label, kuzu_n, cozo_n, if ok { "✅" } else { "❌" });
    }
    ...
    if !sample_id.is_empty() {
        eprintln!("\n=== Query Performance (10 iterations, sample={}) ===", sample_id.rsplit("::").next().unwrap_or(sample_id));
        let iters: u128 = 10;

        let queries: Vec<(&str, Box<dyn Fn() -> Result<()>>, Box<dyn Fn() -> Result<()>>)> = vec![
            ("symbols_in_file",
                Box::new(|| { kq.symbols_in_file(sample_file)?; Ok(()) }),
                Box::new(|| { cozo.symbols_in_file(sample_file)?; Ok(()) })),
            ("callers_of",
                Box::new(|| { kq.callers_of(sample_id)?; Ok(()) }),
                Box::new(|| { cozo.callers_of(sample_id)?; Ok(()) })),
            ("callees_of",
                Box::new(|| { kq.callees_of(sample_id)?; Ok(()) }),
                Box::new(|| { cozo.callees_of(sample_id)?; Ok(()) })),
            ("find_symbol_by_id",
                Box::new(|| { kq.find_symbol_by_id(sample_id)?; Ok(()) }),
                Box::new(|| { cozo.find_symbol_by_id(sample_id)?; Ok(()) })),
            ("get_api_surface",
                Box::new(|| { kq.get_api_surface()?; Ok(()) }),
                Box::new(|| { cozo.get_api_surface()?; Ok(()) })),
            ("get_test_coverage",
                Box::new(|| { kq.get_test_coverage()?; Ok(()) }),
                Box::new(|| { cozo.get_test_coverage()?; Ok(()) })),
            ("transitive_impact",
                Box::new(|| { kq.transitive_impact(sample_id, 3)?; Ok(()) }),
                Box::new(|| { cozo.transitive_impact(sample_id, 3)?; Ok(()) })),
            ("find_all_references",
                Box::new(|| { kq.find_all_references(sample_id)?; Ok(()) }),
                Box::new(|| { cozo.find_all_references(sample_id)?; Ok(()) })),
            ("get_file_deps",
                Box::new(|| { kq.get_file_deps(sample_file)?; Ok(()) }),
                Box::new(|| { cozo.get_file_deps(sample_file)?; Ok(()) })),
            ("symbols_in_range",
                Box::new(|| { kq.symbols_in_range(sample_file, 1, 50)?; Ok(()) }),
                Box::new(|| { cozo.symbols_in_range(sample_file, 1, 50)?; Ok(()) })),
            ("branches_of",
                Box::new(|| { kq.branches_of(sample_id)?; Ok(()) }),
                Box::new(|| { cozo.branches_of(sample_id)?; Ok(()) })),
            ("get_type_hierarchy",
                Box::new(|| { kq.get_type_hierarchy(sample_id, 3)?; Ok(()) }),
                Box::new(|| { cozo.get_type_hierarchy(sample_id, 3)?; Ok(()) })),
            ("generate_test_context",
                Box::new(|| { kq.generate_test_context(None, 10)?; Ok(()) }),
                Box::new(|| { cozo.generate_test_context(None, 10)?; Ok(()) })),
            ("stats",
                Box::new(|| { kuzu.stats()?; Ok(()) }),
                Box::new(|| { cozo.stats()?; Ok(()) })),
        ];

        for (name, kuzu_fn, cozo_fn) in &queries {
            let t0 = Instant::now();
            for _ in 0..iters { kuzu_fn()?; }
            let kuzu_us = t0.elapsed().as_micros() / iters;
            let t0 = Instant::now();
            for _ in 0..iters { cozo_fn()?; }
            let cozo_us = t0.elapsed().as_micros() / iters;
            let pct = if kuzu_us > 0 { (cozo_us as f64 - kuzu_us as f64) / kuzu_us as f64 * 100.0 } else { 0.0 };
            eprintln!("{:<22} Kuzu={:>6}µs  CozoDB={:>6}µs  {:+.0}%", name, kuzu_us, cozo_us, pct);
        }
    }
    ...
    if let Some(ext) = extractions.first() {
        ...
        let t0 = Instant::now();
        for _ in 0..iters { kuzu.upsert_file_conn(&conn, ext)?; }
        let kuzu_inc = t0.elapsed().as_micros() / iters;

        let t0 = Instant::now();
        for _ in 0..iters { cozo.upsert_file(ext)?; }
        let cozo_inc = t0.elapsed().as_micros() / iters;

        let pct = if kuzu_inc > 0 { (cozo_inc as f64 - kuzu_inc as f64) / kuzu_inc as f64 * 100.0 } else { 0.0 };
        eprintln!("upsert_file            Kuzu={:>6}µs  CozoDB={:>6}µs  {:+.0}%", kuzu_inc, cozo_inc, pct);
    }
...
fn print_perf(label: &str, kuzu_ms: u128, cozo_ms: u128) {
    let speedup = if cozo_ms > 0 { kuzu_ms as f64 / cozo_ms as f64 } else { f64::INFINITY };
    eprintln!("{:<15} {:>6}ms {:>6}ms  {:.1}x", label, kuzu_ms, cozo_ms, speedup);
}
